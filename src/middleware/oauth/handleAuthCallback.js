...
export default handler => {
  return async (req, res) => {
    ...
    try {
      ...
      if(!redirect) {
        res.redirect(`${process.env.HOME_PATH}?${querystring.stringify(req.query)}`);
        return;
      }
      ...
      if(redirectUrl.origin) {
        res.redirect(
          `${redirectUrl.origin}${redirectUrl.pathname}?${querystring.stringify({...req.query, ...redirectUrlQuery})}`
        );
        return;
      }

      throw new Error(`Please provide a fully qualifed URL. Instead received ${redirect}`);
